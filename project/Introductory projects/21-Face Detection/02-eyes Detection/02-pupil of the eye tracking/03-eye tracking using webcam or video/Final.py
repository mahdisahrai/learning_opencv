...
while (cap.isOpened()):
    ...
    if ret == True:
        ...
        face_img, bbox = detector.findFaces(frame) # face
        face_img, faces = meshdetector.findFaceMesh(frame) # landmarks
        print(bbox)

        if bbox:
            if faces:

                # left_eye_points: obtaining coordinate of LEFT_EYE(num of landmark)
                left_eye_points = np.array([[faces[0][p][0], faces[0][p][1]] for p in LEFT_EYE])
                # cv.fillPoly(face_img2, pts=[left_eye_points], color = 255) #fillPoly will draw and filling
